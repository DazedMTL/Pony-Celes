...
(function () {
  ...
  var preloadSystemSFX = Boolean(
    parameters["Preload System SFX"] === "true" || false
  );
  var debug = Boolean(parameters["Print Debug to Console"] === "true" || false);
  if (debug)
    console.log("========= TDDP PreloadManager: Debug mode on =========");
  ...
  PreloadManager.preloadImages = function (type, filename, hue) {
    if (filename.constructor === Array) {
      ...
      if (filename.length > 0) {
        ...
        var func =
          "load" + type.charAt(0).toUpperCase() + type.substr(1).toLowerCase();
        if (typeof ImageManager[func] === "undefined") {
          e =
            "PreloadManager: " +
            type +
            " is not a valid image load key. Check your configuration.";
          Graphics.printError(e);
          throw new Error(e);
        }
        this._increaseFileNums();
        ImageManager[func](filename, hue).addLoadListener(
          this.onFileLoaded.bind(this, filename)
        );
      }
    }
  };
  ...
  PreloadManager.preloadAudio = function (type, audioObject) {
    ...
    if (audioObject.constructor === Array) {
      ...
      if (this.isAudioCached(type, audioObject.name)) {
        if (debug)
          console.log(
            type + "/" + audioObject.name + " already preloaded; skipping."
          );
        return;
      }
      this._increaseFileNums();
      var bufferObject = AudioManager.createBuffer(type, audioObject.name);
      bufferObject.addLoadListener(
        this.onFileLoaded.bind(this, bufferObject.name)
      );
      bufferObject.addErrorListener(
        this.onAudioFileError.bind(this, bufferObject)
      );
  ...
  PreloadManager.preloadMapResources = function (mapId) {
    ...
    if (!DataManager.isMapDataLoaded())
      return setTimeout(this.preloadMapResources.bind(this, mapId), 250);
  ...
  Scene_Boot.prototype._performPreload = function () {
    if (!DataManager.isDatabaseLoaded())
      return setTimeout(this._performPreload.bind(this), 5);
    if (debug)
      console.log("========= TDDP PreloadManager: Boot Preload =========");
  ...
  DataManager.loadMapData = function (mapId) {
    if (debug)
      console.log("========= TDDP PreloadManager: Map Preload =========");
