...
(() => {
  ...
  if (Utils.RPGMAKER_NAME === "MV") {
    const _ShaderTilemap_updateTransform =
    ...
    ShaderTilemap.prototype.updateTransform = function () {
      ...
      const startY = Math.floor((oy - this._margin) / this._tileHeight);
      this._moveDarkness(
        startX * this._tileWidth - ox,
        startY * this._tileHeight - oy
      );
  ...
  Game_MapLight.prototype.isValid = function () {
    return (
      (this._radius > 0 || this._isFront) && this._opacity > 0 && this.rgb()
    );
  };
    ...
    for (const command of this.list()) {
      switch (command.code) {
        ...
        case 408:
          ...
          if (comment.startsWith("灯り") || comment.startsWith("CharLight")) {
