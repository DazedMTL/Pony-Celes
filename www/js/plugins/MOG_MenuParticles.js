...
Moghunter.mpart_selfpart = String(
  Moghunter.parameters["Unique Particles"] || "false"
);
Moghunter.mpart_skipscenes = Object(
  Moghunter.parameters["Disable Scenes"] || []
);
...
Moghunter.mpart_number = Number(
  Moghunter.parameters["Number of Particles"] || 15
);
...
Scene_MenuBase.prototype.skip_particles = function () {
  ...
  $gameSystem.get_par_array(
    this._mb_skip_scenes,
    Moghunter.mpart_skipscenes,
    0
  );
  for (var i = 0; i < this._mb_skip_scenes.length; i++) {
    if (this._mb_skip_scenes[i] === SceneManager._scene.constructor.name) {
      return true;
    }
  }
  return false;
...
Scene_MenuBase.prototype.set_particle_img = function () {
  if (this._self_par && SceneManager._scene) {
    return SceneManager._scene.constructor.name + "_par";
  }
  return "Particles";
};
...
Scene_MenuBase.prototype.create_mparticles = function () {
  ...
  if (String(Moghunter.mpart_selfpart) === "true") {
    this._self_par = true;
  }
  this._sprite_particles = [];
  this._sprite_particles_data = [];
  this._nw = [0, 0];
  if (Moghunter.mpart_ox > 0) {
    this._nw[0] = -(Graphics.boxWidth / 3);
  }
  if (Moghunter.mpart_ox < 0) {
    this._nw[0] = Graphics.boxWidth / 3;
  }
  ...
  for (i = 0; i < Moghunter.mpart_number; i++) {
    this._sprite_particles.push(
      new Sprite(ImageManager.loadMenus(this.set_particle_img()))
    );
...
Scene_MenuBase.prototype.reset_particles = function (i) {
  this._sprite_particles_data[i][0] =
    (Math.random() * 2 + 0.4) * Moghunter.mpart_ox;
  this._sprite_particles_data[i][1] =
    (Math.random() * 2 + 0.4) * Moghunter.mpart_oy;
  ...
  if (Moghunter.mpart_oy < 0) {
    this._sprite_particles[i].y =
      Graphics.boxHeight + this._sprite_particles[i].height * 3;
...
Scene_MenuBase.prototype.update_particles = function () {
  for (var i = 0; i < this._sprite_particles.length; i++) {
    ...
    if (this.need_reset_particles(i)) {
      this.reset_particles(i);
    }
  }
};
...
Scene_MenuBase.prototype.need_reset_particles = function (i) {
  if (
    ...
    -this._nw[1] - this._sprite_particles[i].width * 3
  ) {
    return true;
  }
  if (
    this._sprite_particles[i].x >
    this._nw[1] + Graphics.boxWidth + this._sprite_particles[i].width * 3
  ) {
    return true;
  }
