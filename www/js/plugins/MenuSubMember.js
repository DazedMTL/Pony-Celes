...
(function () {
  ...
  var additionalFollower = tmParams
    ? Number(tmParams["additionalFollower"] || 4)
    : 4;
  ...
  var hasHeader = function () {
    return $gameSystem.TMFollowerIncluded() && $gamePlayer.headerFollower() > 0;
  };

  var hasFooter = function () {
    return $gameSystem.TMFollowerIncluded() && $gamePlayer.footerFollower() > 0;
  };

  var numDispActor = function () {
    var actorFollowerNum = $gameParty.size() - 1 + (hasHeader() ? 1 : 0);
    var ceil = $gameSystem.TMFollowerIncluded()
      ? followerMaxSize()
  ...
  Game_Follower.prototype.actor = function () {
    ...
    if (
      hasFooter() &&
      this._memberIndex ===
        Math.min(followerMaxSize(), numDispActor() + numSubMembers() + 1)
    ) {
    ...
    if (DisplayOnMap && this._memberIndex <= (lastId += numSubMembers())) {
      return subMemberPosition(
        this._memberIndex + numSubMembers() - lastId - 1
      );
    }
    return null;
  };
