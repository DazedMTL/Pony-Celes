...
(function () {
  ...
  Game_Interpreter.prototype.pluginCommandCharacterPictureManager = function (
    command,
    args
  ) {
    switch (getCommandName(command)) {
      ...
      case "CPM_DAMAGE_ENDURANCE":
        $gameActors
          .actor(getArgNumber(args[0], 1))
          .damageEndurance(getArgNumber(args[1], 0));
        break;
      case "耐久回復":
  ...
  Game_Battler.prototype.setCpAnimation = function (
    lowerName,
    upperName,
    frame
  ) {
  ...
  Game_Battler.prototype.getCpAnimation = function () {
    return this._upperPicture
      ? this._cpAnimationNameUpper
      : this._cpAnimationName;
  };

  Game_Battler.prototype.getCpSkill = function (skill) {
    return (
      (this._upperPicture ? this.getCpCloth(skill) : null) ||
      this.getPictureName("ノーマル", skill)
    );
  };

  Game_Battler.prototype.getCpDamage = function () {
    return (
      (this._upperPicture ? this.getCpCloth(null, "ダメージ") : null) ||
      this.getPictureName("ダメージ")
    );
  };
  ...
  Game_Battler.prototype.getCpCloth = function (data, addition) {
    for (var i = 0; i < 10; i++) {
      var pictureName = this.getPictureName(
        "耐久" + String(i * 10) + (addition ? addition : ""),
        data
      );
  ...
  Game_Battler.prototype.getPictureName = function (name, data) {
    ...
    var pictureName = getMetaValue(
      data,
      (this._upperPicture ? "上層" : "") + name
    );
  ...
  Game_Actor.prototype.getEndurance = function () {
    var endurance = 0;
    this.getHasEnduranceArmors().forEach(
      function (armor) {
