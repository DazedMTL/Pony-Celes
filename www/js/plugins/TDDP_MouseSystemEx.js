...
(function () {
  ...
  // Auto change cursors
  TDDP_MouseSystemEx.showTextCursor =
    String(parameters["Show Text Cursor"]) || false;
  TDDP_MouseSystemEx.changeGoldCursor =
    String(parameters["Change Gold Cursor"]) || false;
  TDDP_MouseSystemEx.changeItemCursor =
    String(parameters["Change Items Cursor"]) || false;
  TDDP_MouseSystemEx.changeWeaponCursor =
    String(parameters["Change Weapons Cursor"]) || false;
  TDDP_MouseSystemEx.changeArmorCursor =
    String(parameters["Change Armors Cursor"]) || false;
  TDDP_MouseSystemEx.transferPlayerCursor =
    String(parameters["Transfer Cursor"]) || false;
  // Auto change icons
  TDDP_MouseSystemEx.showTextIcon =
    String(parameters["Show Text Icon"]) || false;
  TDDP_MouseSystemEx.changeGoldIcon =
    String(parameters["Change Gold Icon"]) || false;
  TDDP_MouseSystemEx.changeItemIcon =
    String(parameters["Change Items Icon"]) || false;
  TDDP_MouseSystemEx.changeWeaponIcon =
    String(parameters["Change Weapons Icon"]) || false;
  TDDP_MouseSystemEx.changeArmorIcon =
    String(parameters["Change Armors Icon"]) || false;
  TDDP_MouseSystemEx.transferPlayerIcon =
    String(parameters["Transfer Icon"]) || false;
  // Settings
  TDDP_MouseSystemEx.highlightOnHover = Boolean(
    parameters["Highlight On Hover"] === "true" || false
  );
  TDDP_MouseSystemEx.audioCooldownOnHover = Number(
    parameters["Hover SE Cooldown"] || 4
  );
  TDDP_MouseSystemEx.hideCursor = Boolean(
    parameters["Hide Cursor"] === "true" || false
  );
  ...
  TDDP_MouseSystemEx.noAutoCursorNotetag = String(
    parameters["No Auto Cursor Notetag"]
  );
  TDDP_MouseSystemEx.noAutoIconNotetag = String(
    parameters["No Auto Icon Notetag"]
  );
  ...
  TDDP_MouseSystemEx.useCustomCursor = Boolean(
    parameters["Use Custom Cursor?"] === "true" || false
  );
  TDDP_MouseSystemEx.cursorImage = TDDP_MouseSystemEx._ext(
    String(parameters["Custom Cursor Image"])
  );
  TDDP_MouseSystemEx.defaultCursorImage = TDDP_MouseSystemEx.cursorImage;
  TDDP_MouseSystemEx.customCursorPath = String(
    parameters["Custom Cursors Folder"]
  );
  ...
  if (StorageManager.isLocalMode() && Utils.isOptionValid("test")) {
    ...
    var path = window.location.pathname.replace(
      /\/[^\/]*$/,
      "/" + TDDP_MouseSystemEx.customCursorPath
    );
    if (path.match(/^\/([A-Z]\:)/)) {
      path = path.slice(1);
    }
    ...
    if (!fs.existsSync(path)) {
      alert(
        'TDDP MouseSystemEx\nThe chosen cursor folder "' +
          TDDP_MouseSystemEx.customCursorPath +
          '" has been created for you. Please put any custom cursor image files in this folder.'
      );
      fs.mkdirSync(path);
    }
    // Read dir
    ...
    // Store in json
    fs.writeFile(
      path + TDDP_MouseSystemEx._indexFilename,
  ...
  function _loadAndSetupCustomCursorCSS() {
    ...
    var url =
      TDDP_MouseSystemEx.customCursorPath + TDDP_MouseSystemEx._indexFilename;
    xhr.open("GET", url);
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    if (command === "SetCustomCursor")
      TDDP_MouseSystemEx._setCustomCursor(args);
    if (command === "ResetCustomCursor")
   ...
   */
  TDDP_MouseSystemEx._findInEventNotetags = function (
    game_event,
    notetag,
    onMatch
  ) {
    ...
    if (foundMatch) {
      onMatch.call(game_event, result);
    }
  };
  /**
  ...
  TDDP_MouseSystemEx.autoCursorFilters = [
    ...
    [
      TDDP_MouseSystemEx.transferPlayerCursor,
      TDDP_MouseSystemEx._filterTransferPlayer,
    ],
    [TDDP_MouseSystemEx.changeGoldCursor, TDDP_MouseSystemEx._filterChangeGold],
    [
      TDDP_MouseSystemEx.changeItemCursor,
      TDDP_MouseSystemEx._filterChangeItems,
    ],
    [
      TDDP_MouseSystemEx.changeWeaponCursor,
      TDDP_MouseSystemEx._filterChangeWeapons,
    ],
    [
      TDDP_MouseSystemEx.changeArmorCursor,
      TDDP_MouseSystemEx._filterChangeArmors,
    ],
  ...
  if (TDDP_MouseSystemEx.highlightOnHover) {
    ...
    SoundManager.playCursor = function () {
      var _canPlay =
        ...
        Graphics.frameCount >
          SoundManager._lastPlayCursorFrame +
            TDDP_MouseSystemEx.audioCooldownOnHover;
  ...
  TouchInput._setupCursorIconObject = function () {
    ...
    this.cursorIcon.bitmap = new Bitmap(
  ...
  TouchInput._checkCursorStatus = function (pageX, pageY) {
    ...
    if (this.cursorIcon.iconIndex) {
      this.cursorIcon.x =
        Graphics.pageToCanvasX(pageX) +
        (this.cursorIcon.customOffsetX !== null
          ? this.cursorIcon.customOffsetX
          : TDDP_MouseSystemEx.iconOffsetX);
      this.cursorIcon.y =
        Graphics.pageToCanvasY(pageY) +
        (this.cursorIcon.customOffsetY !== null
          ? this.cursorIcon.customOffsetY
          : TDDP_MouseSystemEx.iconOffsetY);
    ...
    if (
      this._activeEvent &&
      this._activeEvent.TDDP_MS.leaveActivate &&
    ...
    if (TDDP_MouseSystemEx.conditionsValidForMouseHoverCheck()) {
      ...
      if (_events.length > 0) {
        ...
        if (game_event.TDDP_MS.hoverIcon) {
          ...
          if (TDDP_MouseSystemEx.hideCursor)
        ...
        if (
          game_event.TDDP_MS.customOffsetX &&
          game_event.TDDP_MS.customOffsetY
        ) {
          TouchInput.cursorIcon.customOffsetX =
            game_event.TDDP_MS.customOffsetX;
          TouchInput.cursorIcon.customOffsetY =
            game_event.TDDP_MS.customOffsetY;
        }
        if (TDDP_MouseSystemEx.useCustomCursor) {
          if (game_event.TDDP_MS.customCursor) {
            TDDP_MouseSystemEx._showCustomCursor(
              game_event.TDDP_MS.customCursor
            );
          } else {
            TDDP_MouseSystemEx._showCustomCursor();
          }
        ...
        if (game_event.TDDP_MS.hoverSwitch) {
          var key = [
            $gameMap._mapId,
            game_event._eventId,
            game_event.TDDP_MS.hoverSwitch.key,
          ];
          $gameSelfSwitches.setValue(
            key,
            game_event.TDDP_MS.hoverSwitch.val === "true"
          );
        }
        return game_event;
      }
      ...
      if (_events.length > 0) {
        ...
        if (game_event.TDDP_MS.clickSwitch) {
          var key = [
            $gameMap._mapId,
            game_event._eventId,
            game_event.TDDP_MS.clickSwitch.key,
          ];
          $gameSelfSwitches.setValue(
            key,
            game_event.TDDP_MS.clickSwitch.val === "true"
          );
          found_click_event = true;
        }
      }
  ...
  Game_Event.prototype.setupMouseSystemProperties = function () {
    ...
    TDDP_MouseSystemEx._findInEventNotetags(
      this,
      /hover_icon\s(.*?);/i,
      function (result) {
        if (!result) return;
    ...
    TDDP_MouseSystemEx._findInEventNotetags(
      this,
      TDDP_MouseSystemEx.noAutoCursorNotetag,
      function () {
    ...
    TDDP_MouseSystemEx._findInEventNotetags(
      this,
      TDDP_MouseSystemEx.noAutoIconNotetag,
      function () {
    ...
    TDDP_MouseSystemEx._findInEventNotetags(
      this,
      TDDP_MouseSystemEx.clickToActivateNote,
      function () {
    ...
    TDDP_MouseSystemEx._findInEventNotetags(
      this,
      TDDP_MouseSystemEx.hoverToActivateNote,
      function () {
    ...
    TDDP_MouseSystemEx._findInEventNotetags(
      this,
      TDDP_MouseSystemEx.leaveToActivateNote,
      function () {
    ...
    if (this.TDDP_MS.allowAutoCursor) {
      for (
        var i = 0, max = TDDP_MouseSystemEx.autoCursorFilters.length;
        i < max;
        i++
      ) {
    ...
    if (this.TDDP_MS.allowAutoIcon) {
      for (
        var i = 0, max = TDDP_MouseSystemEx.autoIconFilters.length;
        i < max;
        i++
      ) {
