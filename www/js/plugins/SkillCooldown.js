...
(() => {
  ...
  const settings = {
    skillCooldowns: JSON.parse(pluginParameters.skillCooldowns || "[]").map(
      (e) => {
        return ((parameter) => {
    ...
    displayCooldown:
      String(pluginParameters.displayCooldown || true) === "true",
    cooldownFormat: String(pluginParameters.cooldownFormat || "CT:{turn}"),
    cooldownColor: Number(pluginParameters.cooldownColor || 2),
    decreaseBenchwarmersCooldown:
      String(pluginParameters.decreaseBenchwarmersCooldown || true) === "true",
  };
  ...
  class SkillCooldown {
    ...
    static setup(triggerSkillId) {
      const cooldownSetting = settings.skillCooldowns.find(
        ...
        ? cooldownSetting.targets.map(
            (target) =>
              new SkillCooldown(
                target.skillId,
                target.turnCount +
                  $gameVariables.value(target.turnCountVariable) +
                  1
              )
          )
        : [];
  ...
  class SkillCooldownManager {
    ...
    setupCooldownTurn(id, skill, isActor) {
      const targetCooldowns = isActor
        ? this.actorsCooldowns(id)
        : this.enemysCooldowns(id);
    ...
    decreaseCooldownTurns() {
      ...
      const actorsCooldowns = settings.decreaseBenchwarmersCooldown
        ? this._actorsSkillCooldowns
        : this._actorsSkillCooldowns.filter((_, actorId) =>
            $gameActors.actor(actorId).isBattleMember()
          );
      const cooldowns = actorsCooldowns
        .flat()
  ...
  function Game_BattlerBase_SkillCooldownMixIn(gameBattlerBase) {
    ...
    gameBattlerBase.meetsSkillConditions = function (skill) {
      return (
        _meetsSkillConditions.call(this, skill) && !this.isDuringCooldown(skill)
      );
    };
    ...
    gameBattlerBase.setupCooldownTurn = function (skill) {
      skillCooldownManager.setupCooldownTurn(
        this.skillCooldownId(),
        skill,
        this.isActor()
      );
    };
    ...
    gameBattlerBase.isDuringCooldown = function (skill) {
      return skillCooldownManager.isDuringCooldown(
        this.skillCooldownId(),
        skill,
        this.isActor()
      );
    ...
    gameBattlerBase.cooldownTurn = function (skill) {
      return skillCooldownManager.cooldownTurn(
        this.skillCooldownId(),
        skill,
        this.isActor()
      );
  ...
  Window_SkillList.prototype.drawSkillCost = function (skill, x, y, width) {
    if (
      $gameParty.inBattle() &&
      settings.displayCooldown &&
      this._actor.isDuringCooldown(skill)
    ) {
      const cooldownText = settings.cooldownFormat.replace(
        /\{turn\}/gi,
        this._actor.cooldownTurn(skill)
      );
