...
(function () {
  if (Imported["MVCommons"] || Imported["PluginManagement"]) {
    ...
    var v = PluginManager.register(
      "GameFocus",
      "1.0.0",
      PluginManager.getBasicPlugin("GameFocus").description,
      author,
      "2015-10-12"
    );
    if (v === undefined) {
      ...
      throw new Error(
        "Unable to load GameFocus due to registration failure! Is there another version running?"
      );
    }
  } else {
    Imported["GameFocus"] = "1.0.0";
    console.log(
      "Neither MVCommons nor PluginManagement are imported - Please install the MVCommons script to maintain an updated list of scripts in the project."
    );
  }
});
...
(function ($) {
  ...
  $.registerCallback = function (type, cb) {
    if (
      typeof cb === "function" &&
      ["onPause", "whilePaused", "onResume"].contains(type)
    ) {
