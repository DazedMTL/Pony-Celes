...
(function () {
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    var args = args || [];
    if (command === "BindPictureToMap")
      this.bindPictureToMap(args[0], true, args[1]);
    if (command === "UnbindPictureFromMap")
      this.bindPictureToMap(args[0], false, args[1]);
    if (command === "ChangePictureLayer")
      this.changePictureLayer(args[0], args[1]);
  ...
  Game_Interpreter.prototype.bindPictureToMap = function (
    pictureId,
    bindToMap,
    layer
  ) {
    ...
    if (!pictureId)
      alert(
        "BindPictureToMap requires the first parameter to be a valid picture Id"
      );
    if (layer) {
    ...
    var picture_sprite =
      SceneManager._scene._spriteset._pictureStorage[pictureId];
    if (picture_sprite) {
  ...
  Game_Interpreter.prototype.bindAllPicturesToMap = function (args) {
    ...
    if (args && args[0]) {
      ...
      if (!SceneManager._scene._spriteset._pictureContainer[layer]) {
        throw new Error(
          "BindPicturesToMap: " + args[0] + " is not a valid layer."
        );
      }
    ...
    if (picture) {
      if (picture.layer()) {
        if (picture.layer() != parent) {
          ...
          picture.layer().children.sort(function (a, b) {
  ...
  Sprite_Picture.prototype._useLoopingBitmap = function () {
    return (
      this.picture()._bindToMap &&
      ($gameMap.isLoopHorizontal() || $gameMap.isLoopVertical())
    );
  };
  ...
  Game_Picture.prototype.show = function (
    ...
    this._mapOffsX = Math.max(
      SceneManager._screenWidth - $gameMap.width() * $gameMap.tileWidth(),
      0
    );
    this._mapOffsY = Math.max(
      SceneManager._screenHeight - $gameMap.height() * $gameMap.tileHeight(),
      0
    );
  ...
  Game_Picture.prototype.setDimensions = function (bitmap) {
    ...
    this._bitmapWidth = this._useHorizontalRepeat
      ? this._horSpacing * 2
      : this._width;
    this._bitmapHeight = this._useVerticalRepeat
      ? this._verSpacing * 2
      : this._height;
  };
  /**
